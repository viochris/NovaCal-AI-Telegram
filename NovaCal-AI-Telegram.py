...
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN_Nova_cal")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    interacts with the bot. It welcomes the user and introduces NovaCal AI.
    ...
    # 2. Construct the welcoming interface text with clear usage instructions
    welcome_text = (
        ...
        "🧠 **1. Zero-Memory Engine (Stateless)**\n"
        "I process each message independently and **do not remember previous chats**. Please include all instructions in a single message. "
        "*(e.g., Instead of sending 'I have a meeting' and then 'at 4 PM' separately, say 'Schedule a meeting today at 4 PM'.)*\n\n"
        
        "⏱️ **2. Provide Complete Details**\n"
        "To minimize errors, always provide complete info. If you ask me to create an event but don't specify the duration, "
        "I might automatically set it to a **1-hour default** or ask you for clarification. *(Don't worry, you can always update it later!)*\n\n"
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    displaying its technical specifications, underlying architecture, 
    ...
    info_text = (
        ...
        "• **Architecture:** Stateless (Zero-Memory)\n"
        "• **Security:** Private Access Control & Activity Logging\n"
        "• **Developers:** Silvio Christian, Joe\n\n"
        
        "⚡ **WHY STATELESS?**\n"
        "By operating without persistent memory, NovaCal AI ensures **maximum token efficiency, minimal hallucinations, and lightning-fast response times**. It is highly optimized for accurate, single-turn command execution.\n\n"
...
async def howtouse_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    It delivers a comprehensive guide on the operational rules (Stateless nature)
    ...
    howtouse_text = (
        ...
        "🧠 **1. THE ONE-SHOT RULE (NO MEMORY)**\n"
        "I process every command as a brand new task. I **do not remember** previous messages. You must provide all necessary details in a **single message**.\n"
        "❌ *Bad:* 'Schedule a meeting.' -> (Next message) -> 'Tomorrow at 3 PM.'\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try: 
        ...
        # 7. Initialize the Runtime Executor (The Body) without persistent memory
        ...
        # 8. Execute the Agent workflow
        response = agent_executor.invoke({"input": user_text})

        # 9. Safely parse the LLM's output
        ...
        # 10. Handle Telegram's Message Length Limits
        ...
        # 11. Transmit the formulated chunks back to the user asynchronously
        ...
        # 12. Prevent silent failures by safely catching, categorizing, and notifying the user
